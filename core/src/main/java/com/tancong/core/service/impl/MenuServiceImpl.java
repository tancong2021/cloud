...
import com.tancong.core.mapper.MenuMapper;
...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    public void refreshMenus() {
        CacheManagers.set("MenuList", this.list());
    }
    ...
    public List<Menu> getAll(boolean includePermission) {
        List<Menu> list = (List<Menu>) CacheManagers.get("MenuList");
        if (Objects.isNull(list) || list.isEmpty()) {
            list = menuMapper.selectList(null);
            CacheManagers.set("MenuList", list);
        }
        // 按照规则取出菜单
